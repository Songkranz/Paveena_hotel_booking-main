...
$sort_options = [
    'latest' => ['label' => 'ล่าสุดก่อน', 'order' => 'b.created_at DESC, b.id DESC'],
    'oldest' => ['label' => 'เก่าสุดก่อน', 'order' => 'b.created_at ASC, b.id ASC'],
    'name' => ['label' => 'ชื่อผู้จอง ก-ฮ / A-Z', 'order' => 'u.full_name ASC, b.created_at DESC']
];

if (!isset($sort_options[$sort])) {
    $sort = 'latest';
}
$recent_order = $sort_options[$sort]['order'];
$sort_label = $sort_options[$sort]['label'];
    ...
    <main>
        ...
        <section class="report-body-zone">
            ...
            <div class="report-two-column">
                <section class="report-section report-revenue-breakdown">
                    ...
                    <?php else: ?>
                        <div class="report-revenue-grid">
                            <?php foreach ($by_type as $index => $row): ?>
                                <?php
                                $share = $total_revenue > 0 ? ((float)$row['rev'] / $total_revenue) * 100 : 0;
                                $type_average = (int)$row['cnt'] > 0 ? (float)$row['rev'] / (int)$row['cnt'] : 0;
                                ?>
                                <article class="report-revenue-card">
                                    <div class="report-revenue-card-top">
                                        <span>ประเภทที่ <?= $index + 1 ?></span>
                                        <strong><?= number_format($share, 1) ?>%</strong>
                                    </div>
                                    <h3><?= htmlspecialchars($row['type_name']) ?></h3>
                                    <div class="report-revenue-amount">
                                        <strong><?= number_format((float)$row['rev'], 2) ?></strong>
                                        <span>บาท</span>
                                    </div>
                                    <div class="report-revenue-meta">
                                        <span><?= number_format((int)$row['cnt']) ?> การจอง</span>
                                        <span>เฉลี่ย <?= number_format($type_average, 2) ?> บาท/การจอง</span>
                                    </div>
                                </article>
                            <?php endforeach; ?>
                        </div>
            ...
            <section class="report-section report-print-status">
                    <div class="report-section-heading">
                        <div>
                            <h2>สถานะการจอง</h2>
                        </div>
            ...
            <section class="report-section report-recent">
                <div class="report-section-heading">
                    ...
                    <div class="report-sort-controls">
                        <form class="report-sort-form" method="GET">
                            <input type="hidden" name="start_date" value="<?= htmlspecialchars($start) ?>">
                            <input type="hidden" name="end_date" value="<?= htmlspecialchars($end) ?>">
                            <label>
                                <span>เรียงตาม</span>
                                <select name="sort" aria-label="เรียงรายการจอง">
                                    <option value="latest" <?= $sort === 'latest' ? 'selected' : '' ?>>ล่าสุดก่อน</option>
                                    <option value="oldest" <?= $sort === 'oldest' ? 'selected' : '' ?>>เก่าสุดก่อน</option>
                                    <option value="name" <?= $sort === 'name' ? 'selected' : '' ?>>ชื่อผู้จอง ก-ฮ / A-Z</option>
                                </select>
                            </label>
                            <button type="submit">เรียงข้อมูล</button>
                        </form>
                        <span class="report-sort-print"><?= htmlspecialchars($sort_label) ?> · สูงสุด 10 รายการ</span>
                    </div>
                </div>
                <?php if (!$recent): ?>
                ...
                <?php else: ?>
                    <div class="report-table-wrap">
                        <table class="report-table">
                            ...
                            <tbody>
                            <?php foreach ($recent as $display_index => $row): ?>
                                <tr>
